...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	insulator: {
		onSourceModifyDamage(damage, source, target, move) {
			if (target.getMoveHitData(move).typeMod > 0) {
				this.debug('Insulator neutralize');
				return this.chainModify(0.75);
			}
		},
		flags: {breakable: 1},
	...
	leadenfist: {
		onStart(pokemon) {
			pokemon.addVolatile('leadenfist');
		},
		onEnd(pokemon) {
			delete pokemon.volatiles['leadenfist'];
			this.add('-end', pokemon, 'Leaden Fist', '[silent]');
		},
		condition: {
			duration: 3,
			onResidualOrder: 28,
			onResidualSubOrder: 2,
			onStart(target) {
				this.add('-start', target, 'ability: Leaden Fist');
			},
			onResidual(pokemon) {
			...
			onEnd(target) {
				this.add('-end', target, 'Leaden Fist');
			},
		},
